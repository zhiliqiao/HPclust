...
Est_q = function(n, Treatment, Z_mat){
  ...
  q = 0.5 + (q - 0.5) * (1-1e-10) ### get rid of 0's and 1's
  q
}
...
# hurdle poisson log-likelihood for gene g in cluster k
#' Title
#'
#' @param Treatment a
#' @param s a
...
l_hp = function(Treatment, s, ng, mu, alpha, q){
  ### s:  vector, normalization factor, length IJ
  ### ng: vector, the g_th row of data, length IJ
  ### mu: vector, the k_th cluster center, length I
  ### alpha: number, mean expression level
  ### q:  vector, prob of non-zero for cluster k, length I
  t = as.vector((summary(as.factor(Treatment)))[as.factor(unique(Treatment))])
...
# Expectation of Z_gk for gene g
#' Title
#'
#' @param Treatment a
#' @param sg a
...
Z_g = function(Treatment, sg, ng, mu, alpha_g, q, p){
  ### sg: vector of normalization factor, length IJ
  ### ng: vector, g_th row of the data, length IJ
  ### mu: matrix, cluster centers, K*I
  ### alpha_g: vector, mean expression level of gene g, length K
  ### q: matrix, prob of poisson state, K*I
  ### P: vector, mixing proportion, length K
...
Expect_Z = function(Treatment, s, n, mu, alpha, p, q){
  ### s: matrix of normalization factor, G*IJ
  ### n: matrix of the data, G*IJ
...
# hurdle poisson log-likelihood for cluster k
#' Title
#'
#' @param Treatment a
#' @param s a
...
# hurdle poisson log-likelihood for cluster k, Treatment i (as function of mu_ki & alpha)
#' Title
#'
#' @param Treatment a
#' @param s a
...
find_norm = function(data, Treatment){
  # data: Dataset
  # Treatment: Vector of treatments, eg. (1,1,1,2,2,2,3,3,3,...)
...
initial = function(mydata, nK, dis){
  ...
  for (i in 1:(length(i0)-1)){
    ...
    # proportion of non-zeros for initial center k, treatment i
    q0[, i] = 1 - apply(temp==0, 1, sum)/ncol(temp)
    q0 = 0.5 + (q0 - 0.5) * (1-1e-10) # get rid of 0 & 1's
...
#' @importFrom stats cor optimize pchisq quantile rmultinom
#' @noRd
hp_cluster = function(mydata, q0, mu0, method = c('EM', 'SA'), cool = 0.9){
  s = mydata$Normalizer
  n = mydata$Count
  Treatment = mydata$Treatment

  p0 = rep(1/nrow(mu0), nrow(mu0))
  alpha0 = Est_alpha(s, n, mu0, Treatment)
  ...
  repeat{
    ...
    Z_list[[iter]] = Z_mat
    #print(summary(as.factor(apply(Z_mat, 1, which.max))))
    #print(NMI(apply(Z_mat, 1, which.max), true_k))
    ...
    if(iter == 30 | prop_diff<1e-5) break
  }
...
# function for finding MLE for a dataset
#' Title
#'
#' @param n a
#' @param s a
